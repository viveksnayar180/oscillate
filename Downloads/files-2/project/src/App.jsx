...
// ─────────────────────────────────────────────────────────────────────────────
// 1M CANDLE GENERATOR — session-aware, AMD-cycle biased
// All higher TFs are derived from this via aggregateCandles().
// ─────────────────────────────────────────────────────────────────────────────
function generate1MCandles(ins="NQ", count=800) {
  const nq = ins === "NQ";
  const vol1m = nq ? 3.5 : 1.2;   // per-1M average move
  let p = (BASE_PRICES[ins] || 19200) + (Math.random() - 0.5) * 40;
  const out = [];
  const now = Date.now();
  const startTime = now - count * 60000;
  ...
  for (let i = 0; i < count; i++) {
    ...
    let vm = 1.0;
    if      (h < 7)              vm = 0.35; // Asia — low
    else if (h < 9)              vm = 1.7;  // London open — hot
    else if (h < 13)             vm = 1.1;  // London main
    else if (h < 15)             vm = 2.2;  // NY open — hottest
    else if (h < 17)             vm = 1.4;  // NY main
    else if (h === 17)           vm = 0.5;  // NY lunch
    else                         vm = 0.3;  // after-hours

    // AMD bias — accumulate early, sweep midday, distribute late
    const pct = (h + m / 60) / 24;
    const bias = pct < 0.33 ? 0.08 : pct < 0.66 ? (Math.random() - 0.5) * 0.3 : -0.04;

    const mv  = (Math.random() - 0.5 + bias) * vol1m * vm;
    const o   = p, c = o + mv;
    const wick = Math.abs(mv) * (0.3 + Math.random() * 0.8);
    const hi   = Math.max(o, c) + wick * Math.random();
    const lo   = Math.min(o, c) - wick * Math.random();

    const baseVol = nq ? 2000 : 800;
    const vol     = Math.floor(baseVol * vm * (0.7 + Math.random() * 0.6));
    const bull    = c >= o;
    const askR    = bull ? 0.52 + Math.random() * 0.28 : 0.22 + Math.random() * 0.26;
    const askVol  = Math.floor(vol * askR);
    const bidVol  = vol - askVol;

    out.push({ t, o, h: hi, l: lo, c, v: vol, delta: askVol - bidVol, askVol, bidVol });
    p = c;
  }
  return out;
}
...
// O=first.o  H=max(highs)  L=min(lows)  C=last.c  V=sum
function aggregateCandles(oneMinCandles, tfMinutes) {
  if (!oneMinCandles?.length) return [];
  if (tfMinutes <= 1) return oneMinCandles;
  const out = [];
  for (let i = 0; i < oneMinCandles.length; i += tfMinutes) {
    const s = oneMinCandles.slice(i, i + tfMinutes);
    ...
    out.push({
      ...
      askVol: s.reduce((a, c) => a + c.askVol, 0),
      bidVol: s.reduce((a, c) => a + c.bidVol, 0),
      delta:  s.reduce((a, c) => a + c.delta, 0),
    });
  }
  return out;
}

// Backward-compat alias (still used in refresh() inside LiveChartTab)
function generateCandles(ins="NQ", count=200) {
  // Return 5M candles (aggregated from 1M) so existing callers get ~same count
  return aggregateCandles(generate1MCandles(ins, count * 5), 5);
}
function genOBs(cs){
  const a=[];
...
// LIVE mode  : one WebSocket per TF → real Tradovate bars
// SIM mode   : single 1M base generated by generate1MCandles(); all higher TFs
//              are derived mathematically via aggregateCandles() so candles are
//              100% correlated (3M = every 3 × 1M bars, 5M = every 5, etc.).
//              A 1.2-second interval ticks the current 1M candle live, and a
//              60-second interval appends a new 1M bar — both cascade to every
//              derived TF via useMemo.
// ─────────────────────────────────────────────────────────────────────────────
const MULTI_TF_LIST = [
  ...
  { tf:"30",  label:"30M", size:30  },
  { tf:"60",  label:"1H",  size:60  },
  { tf:"240", label:"4H",  size:240 },
];

// Aggregation map: tfKey → minutes (used by sim mode)
const TF_MINUTES = { "1":1, "3":3, "5":5, "15":15, "30":30, "60":60, "240":240 };

function useMultiTFCandles({ conn, instrument }) {
  // ── SIM MODE — 1M base + derived higher TFs ──────────────────────────────
  const [baseCandles, setBaseCandles] = useState(
    () => generate1MCandles(instrument || "NQ", 800)
  );

  // Re-seed when instrument changes in sim mode
  useEffect(() => {
    if (conn?.token) return;
    setBaseCandles(generate1MCandles(instrument || "NQ", 800));
  }, [instrument, conn?.token]);

  // Hierarchical TF chain — each higher TF built from its immediate lower parent
  const simTFCandles = useMemo(() => {
    const m1  = baseCandles;
    const m3  = aggregateCandles(m1, 3);           // 3M from 1M  (×3)
    const m5  = aggregateCandles(m1, 5);           // 5M from 1M  (×5) — 5÷3 not integer, stays at 1M
    const m15 = aggregateCandles(m5, 3);           // 15M from 5M (×3)
    const m30 = aggregateCandles(m15, 2);          // 30M from 15M (×2)
    const m60 = aggregateCandles(m30, 2);          // 60M from 30M (×2)
    const h4  = aggregateCandles(m60, 4);          // 4H from 60M  (×4)
    return { "1":m1, "3":m3, "5":m5, "15":m15, "30":m30, "60":m60, "240":h4 };
  }, [baseCandles]);

  // SMT pair instrument — ES when main is NQ, NQ when main is ES, etc.
  const SMT_INS_MAP_LOCAL = { NQ:"ES", ES:"NQ", RTY:"ES", YM:"ES" };
  const pairIns = SMT_INS_MAP_LOCAL[instrument || "NQ"] || "ES";
  const [smtBaseCandles] = useState(() => generate1MCandles(pairIns, 800));

  const smtTFCandles = useMemo(() => {
    const m1  = smtBaseCandles;
    ...
    const m60 = aggregateCandles(m30, 2);
    const h4  = aggregateCandles(m60, 4);
    return { "1":m1, "3":m3, "5":m5, "15":m15, "30":m30, "60":m60, "240":h4 };
  }, [smtBaseCandles]);

  // Live tick — update current 1M candle every 1.2 s
  useEffect(() => {
    if (conn?.token) return;
    ...
    const id = setInterval(() => {
      setBaseCandles(prev => {
        ...
        const mv = (Math.random() - 0.49) * (nq ? 3.5 : 1.2);
        last.c += mv;
        last.h  = Math.max(last.h, last.c);
        last.l  = Math.min(last.l, last.c);
        last.delta = Math.floor((Math.random() - 0.45) * last.v * 0.8);
        arr[arr.length - 1] = last;
        return arr;
      });
    }, 1200);
    return () => clearInterval(id);
  }, [conn?.token, instrument]);

  // New 1M bar every 60 s — keeps chart feeling live
  useEffect(() => {
    if (conn?.token) return;
    const nq = instrument === "NQ";
    const id = setInterval(() => {
      setBaseCandles(prev => {
        ...
        const o   = last.c;
        const mv  = (Math.random() - 0.48) * (nq ? 7 : 2.5);
        const c   = o + mv;
        const h   = Math.max(o, c) + Math.random() * (nq ? 4 : 1.5);
        const l   = Math.min(o, c) - Math.random() * (nq ? 4 : 1.5);
        const vol = Math.floor(1500 + Math.random() * 2500);
        const askVol = Math.floor(vol * (0.45 + Math.random() * 0.2));
        const bidVol = vol - askVol;
        const newBar = { t: Date.now(), o, h, l, c, v: vol,
          askVol, bidVol, delta: askVol - bidVol };
        return [...prev.slice(-799), newBar];
      });
    }, 60000);
    return () => clearInterval(id);
  }, [conn?.token, instrument]);

  // ── LIVE MODE — one WebSocket per TF (Tradovate) ──────────────────────────
  const [liveTFCandles, setLiveTFCandles] = useState({});
  const wsRefs  = useRef({});
  const hbRefs  = useRef({});
      ...
      frames.forEach(frame => {
        ...
        if (frame.e === "chart" && frame.d?.charts) {
          ...
          if (bars.length) {
            ...
            setLiveTFCandles(prev => ({ ...prev, [tf]: candles }));
          }
        }
  ...
  // Return real data when live, sim data otherwise
  const tfCandles = conn?.token ? liveTFCandles : simTFCandles;
  const get = useCallback((tf) => tfCandles[tf] || [], [tfCandles]);

  return { tfCandles, get, baseCandles, setBaseCandles, smtTFCandles };
}
...
function compSMTSignal(mainCs, pairCs, n=20) {
  // True SMT: compare main instrument swings vs SMT pair swings
  // Bull SMT = main makes new low, pair holds higher low (divergence → long)
  // Bear SMT = main makes new high, pair holds lower high (divergence → short)
  if (!mainCs?.length) return { bullSMT:false, bearSMT:false, desc:"No data" };
  const pairAvail = pairCs?.length >= 4;
  const m = mainCs.slice(-n);
  const p = pairAvail ? pairCs.slice(-n) : null;
  if (m.length < 4) return { bullSMT:false, bearSMT:false, desc:"Insufficient data" };

  const mLows  = m.map(c => c.l),  mHighs = m.map(c => c.h);
  const mLastLow   = mLows[mLows.length - 1];
  const mPrevLow   = Math.min(...mLows.slice(0, -3));
  const mLastHigh  = mHighs[mHighs.length - 1];
  const mPrevHigh  = Math.max(...mHighs.slice(0, -3));

  if (p) {
    const pLows  = p.map(c => c.l),  pHighs = p.map(c => c.h);
    const pLastLow   = pLows[pLows.length - 1];
    const pPrevLow   = Math.min(...pLows.slice(0, -3));
    const pLastHigh  = pHighs[pHighs.length - 1];
    const pPrevHigh  = Math.max(...pHighs.slice(0, -3));
    // Bull SMT: main new low, pair does NOT confirm (higher low)
    if (mLastLow < mPrevLow && pLastLow > pPrevLow)
      return { bullSMT:true,  bearSMT:false, desc:"Main new low, pair higher low — institutional accumulation (long)" };
    // Bear SMT: main new high, pair does NOT confirm (lower high)
    if (mLastHigh > mPrevHigh && pLastHigh < pPrevHigh)
      return { bullSMT:false, bearSMT:true,  desc:"Main new high, pair lower high — institutional distribution (short)" };
  } else {
    // Fallback: single-instrument delta divergence proxy
    const deltas = m.map(c => c.delta||0);
    const lastD = deltas[deltas.length-1], prevLowD = Math.min(...deltas.slice(0,-3)), prevHighD = Math.max(...deltas.slice(0,-3));
    if (mLastLow < mPrevLow && lastD > prevLowD)
      return { bullSMT:true,  bearSMT:false, desc:"Price new low, delta diverging UP — accumulation proxy" };
    if (mLastHigh > mPrevHigh && lastD < prevHighD)
      return { bullSMT:false, bearSMT:true,  desc:"Price new high, delta diverging DOWN — distribution proxy" };
  }
  return { bullSMT:false, bearSMT:false, desc:"No SMT divergence detected" };
}
...
// LIVE CHART TAB
// ─────────────────────────────────────────────────────────────────────────────
function LiveChartTab({ onStateChange, tradeSetup, onTradeSetupUpdate, tvConn: tvConnProp, setTvConn: setTvConnProp, lastScanResult: lastScanResultProp, simTFCandles, smtTFCandlesProp }) {
  ...
  const [contextCandles, setContextCandles] = useState([]); // lower-TF context strip
  const [smtCandles,     setSmtCandles]     = useState([]); // SMT pair instrument candles
  ...
  // Recompute overlays whenever the candle array changes (sim OR live)
  const recomputeOverlays = useCallback((c) => {
    if (!c?.length) return;
    const obList = genOBs(c);
    const sh = Math.max(...c.slice(-30).map(x=>x.h));
    const sl = Math.min(...c.slice(-30).map(x=>x.l));
    const of2 = compOrderFlow(c);
    setOBs(obList); setBBs(genBBs(obList,c)); setFVGs(genFVGs(c));
    setLiq(genLiq(c)); setContextLevels(compContextualLevels(c));
    setOrb(compORB(c)); setOte(compOTE(sh,sl));
    setOFData(of2); setVolProf(compVolProfile(c));
  }, []);

  // TF hierarchy for lower-TF context strip
  const TF_LOWER_MAP = { "3":"1", "5":"3", "15":"5", "30":"15", "60":"30", "240":"60" };

  // SIM MODE: sync candles from the correlated MTF set whenever the TF
  // selection or the simTFCandles object changes (driven by the 1M tick).
  useEffect(() => {
    if (wsStatus === "streaming") return; // live WS takes over
    const tfKey = tvInterval || "5";
    const c = simTFCandles?.[tfKey];
    if (c?.length) {
      setCandles(c);
      recomputeOverlays(c);
      setBias(c[c.length-1]?.c >= (c[c.length-2]?.c ?? 0) ? "BULLISH" : "BEARISH");
    }
    // Populate lower-TF context strip
    const lowerKey = TF_LOWER_MAP[tfKey];
    const lc = lowerKey ? simTFCandles?.[lowerKey] : null;
    setContextCandles(lc?.length ? lc.slice(-150) : []);
  }, [simTFCandles, tvInterval, wsStatus, recomputeOverlays]);

  // SMT pair candles — sync from prop whenever TF or prop changes
  useEffect(() => {
    if (wsStatus === "streaming") return;
    const c = smtTFCandlesProp?.[tvInterval];
    if (c?.length) setSmtCandles(c);
  }, [smtTFCandlesProp, tvInterval, wsStatus]);

  // Split layout: auto-follow lower TF so Chart 2 always shows the TF that feeds Chart 1
  useEffect(() => {
    if (chartLayout === "split") {
      const lower = TF_LOWER_MAP[tvInterval];
      if (lower) setSplitTf(lower);
    }
  }, [tvInterval, chartLayout]);

  // Fallback refresh (only used when simTFCandles isn't available yet)
  const refresh = useCallback((i=ins)=>{
    if (wsStatus==="streaming") return;
    if (simTFCandles?.["5"]?.length) return; // sim candles already loaded
    const c=generateCandles(i,200);
    setCandles(c);
    recomputeOverlays(c);
    setBias(Math.random()>0.5?"BULLISH":"BEARISH");
  },[ins, wsStatus, simTFCandles, recomputeOverlays]);
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:0}}>
      ...
      {(()=>{
        ...
        // TF hierarchy — maps each TF to its immediate lower-TF parent
        const TF_LOWER = { "3":"1", "5":"3", "15":"5", "30":"15", "60":"30", "240":"60" };
        const TF_LABEL = { "1":"1M","3":"3M","5":"5M","15":"15M","30":"30M","60":"60M","240":"4H" };
        ...
        if (chartLayout==="single") {
          ...
          return (
            <div style={{
              ...
              {/* ── Lower-TF Context Strip ── shows the TF that feeds into the current TF */}
              {!fullscreen && contextCandles.length > 0 && TF_LOWER[tvInterval] && (
                <div style={{marginTop:4,borderRadius:6,overflow:"hidden",
                  border:"1px solid rgba(0,212,255,0.12)",background:"#080c14"}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,padding:"3px 10px",
                    background:"rgba(0,0,0,0.5)",borderBottom:"1px solid rgba(0,212,255,0.08)"}}>
                    <span style={{fontSize:8,color:"rgba(255,255,255,0.3)",fontFamily:"monospace",
                      letterSpacing:1,textTransform:"uppercase"}}>Context</span>
                    <span style={{fontSize:10,color:"#00d4ff",fontWeight:700,fontFamily:"monospace"}}>
                      {TF_LABEL[TF_LOWER[tvInterval]]}
                    </span>
                    <span style={{fontSize:8,color:"rgba(255,255,255,0.22)",fontFamily:"monospace"}}>
                      feeds → {TF_LABEL[tvInterval]}
                    </span>
                  </div>
                  <CandleChart candles={contextCandles}
                    obs={[]} bbs={[]} fvgs={[]} liq={[]} contextLevels={[]}
                    orb={null} ote={null} overlays={overlays}
                    ofData={null} volProfile={null} tradeSetup={null}/>
                </div>
              )}
            </div>
          );
        ...
        return (
          <div style={{display:"flex",flexDirection:"column",gap:4,marginBottom:10,
            ...
            <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap",
              ...
              {/* SMT signal readout */}
              {candles.length > 0 && (()=>{
                const sig = compSMTSignal(candles, smtCandles);
            ...
            <div style={{display:"flex",flexDirection:"row",gap:4,
              ...
              <div style={{...box("100%","1px solid rgba(192,132,252,0.25)"),flex:1}}>
                ...
                {chartSource==="canvas"
                  ? <CandleChart candles={smtCandles.length ? smtCandles : candles}
                      obs={[]} bbs={[]} fvgs={[]} liq={[]} contextLevels={[]}
                      orb={null} ote={null} overlays={overlays}
                      ofData={null} volProfile={null} tradeSetup={null}/>
...
export default function App() {
  ...
  // ── Multi-TF candles ───────────────────────────────────────────────────
  const { tfCandles, smtTFCandles } = useMultiTFCandles({
    conn:       tvConn,
  ...
  return (
    <div style={{minHeight:"100vh",background:"#080c14",fontFamily:"'Inter',-apple-system,sans-serif",color:"#e2e8f0"}}>
      ...
      <div style={{maxWidth:"1200px",margin:"0 auto",padding:tab==="chart"?"14px 24px":"24px"}}>
        {tab==="chart"&&<LiveChartTab onStateChange={setChartState} tradeSetup={tradeSetup} onTradeSetupUpdate={setTradeSetup} tvConn={tvConn} setTvConn={setTvConn} lastScanResult={lastScanResult} simTFCandles={tfCandles} smtTFCandlesProp={smtTFCandles}/>}
